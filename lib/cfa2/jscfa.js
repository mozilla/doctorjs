...
function fixStm(n, parentScript) {
  ...
  case BLOCK:
    ...
    while (i < ch.length) {
      ...
      case LET_BLOCK:
        n2.varDecls.forEach(function(vd) {
          vd.name = vd.value;
          vd.initializer && (vd.initializer = fixExp(vd.initializer));
        });
        if (n2.expression) { // let-exp in stm context
          n2.expression = fixExp(n2.expression);
          n2.block = semiNode(n2.expression);
          delete n2.expression;
        }
        else // let-stm
          ans1 = fixStm(n2.block, parscr) || ans1;
        i++;
        break;
...
(function () {
  ...
  function _block(n, withAddr, boundvars, vardecls) {
    var i = 0, ch = n.children, n2;
    while (i < ch.length) {
      n2 = ch[i];
      if (n2.type === WITH) {
        var newaddr = newCount(), t = n2.tokenizer, 
        freshvar = withIdNode(t, newaddr), assgn;
        vardecls.push(freshvar);
        assgn = new Node(t, {type:ASSIGN});
        assgn.children = [freshvar, n2.object];
        ch[i++] = semiNode(assgn);
        desugarWith(n2.body, newaddr, [], vardecls);
        ch.splice(i, 0, n2.body);
      }
      else
        desugarWith(n2, withAddr, boundvars, vardecls);
      i++;
    }
  }
  ...
  override(SCRIPT, function(n, withAddr, boundvars) {
    ...
    _block(n, withAddr, boundvars, n.varDecls);
  ...
  override(FOR_IN, function(n, withAddr, boundvars, vardecls) {
    var it = n.iterator, obj = n.object, b = n.body;
...
function tagVarRefsId(classifyEvents) {
  return function(n, innerscope, otherscopes) {
    ...
    nthis.kind = HEAP;
    if (classifyEvents && n.name === "addEventListener")
...
// call to evalFun that hasn't returned, pending contains a triple {a, t, b}
// where a is the args for the call, t is the timestamp at the time of the call,
// and b is a boolean used for recursive functions.
var pending;
...
function initCoreObjs() {
  ...
  // Global object
  var go = new Aobj({}), goaddr = newCount();
  var goav = makeObjAval(goaddr), goavaddr = newCount();
  ...
  // Object.prototype
  var op = new Aobj({}), opaddr = newCount(), opav = makeObjAval(opaddr);
  object_prototype_av = opav;
  heap[opaddr] = op;

  // Object.__proto__ (same as Function.prototype)
  var o_p = new Aobj({proto:opav}), o_paddr = newCount();
  var o_pav = makeObjAval(o_paddr);
  function_prototype_av = o_pav;
  heap[o_paddr] = o_p;

  // Function.prototype.prototype
  var fpp = new Aobj({proto : opav}), fppaddr = newCount();
  ...
  var _Object = (function () {
    ...
    var nonew = new Aobj({proto : opav}), nonewaddr = newCount();
  ...
  var oaddr = newCount(), oav = makeObjAval(oaddr);
  heap[oaddr] = o;
  ...
  // Function
  var f = new Aobj({proto : o_pav}), faddr = newCount();
  ...
  (function () {
    // Array.prototype
    var ap = new Aobj({proto:opav}), apaddr = newCount();
    ...
    var _Array = (function () {
      ...
      var nonew = new Aobj({proto : apav}), nonewaddr = newCount();
      var nonewav = makeObjAval(nonewaddr);
      heap[nonewaddr] = nonew;
    ...
    var aaddr = newCount(), aav = makeObjAval(aaddr);
    heap[aaddr] = a;
  ...
  (function () {
    ...
    var naddr = newCount(), nav = makeObjAval(naddr);
    heap[naddr] = n;
  ...
  (function () {
    // String.prototype
    var sp = new Aobj({proto:opav}), spaddr = newCount();
    ...
    var omatch = new Aobj({}), omatchaddr = newCount();
    ...
    var osplit = new Aobj({}), osplitaddr = newCount();
    ...
    heap[newCount()] = new Aobj({proto : spav});
    ...
    var saddr = newCount(), sav = makeObjAval(saddr);
    heap[saddr] = s;
  ...
  (function () {
    // Error.prototype
    var ep = new Aobj({proto:opav}), epaddr = newCount();
    ...
    var eaddr = newCount(), eav = makeObjAval(eaddr);
    heap[eaddr] = e;
    ...
    var sep = new Aobj({proto:epav}), sepaddr = newCount();
    ...
    var seaddr = newCount(), seav = makeObjAval(seaddr);
    heap[seaddr] = se;
  ...
  (function () {
    ...
    var raddr = newCount(), rav = makeObjAval(raddr);
    heap[raddr] = r;
  ...
  (function () {
    // Date.prototype
    var dp = new Aobj({proto:opav}), dpaddr = newCount();
    ...
    var daddr = newCount(), dav = makeObjAval(daddr);
    heap[daddr] = d;
  ...
  (function () {
    ...
    var maddr = newCount(), mav = makeObjAval(maddr);
    heap[maddr] = m;
  ...
  (function () {
    ...
    var baddr = newCount(), bav = makeObjAval(baddr);
    heap[baddr] = b;
...
// function node, array of Aval -> {args, timestamp, boolean} or number
function searchPending(n, args) {
  var bucket = pending[n.addr], len = bucket.length;
  ...
  for (var i = len - 1; i >= 0; i--)
    if (arrayeq(aveq, args, bucket[i].args)) return bucket[i];
  return 0;
}

// function node, {args, timestamp, boolean} -> void
function addPending(n, elm) { pending[n.addr].push(elm); }

// function node -> {args, timestamp, boolean}
...
function makeGenericObj() {
  var gobj = new Aobj({proto : object_prototype_av}), gobjaddr = newCount();
  heap[gobjaddr] = gobj;
...
function StackCleaner(fn, howmany) {
  this.fn = fn;
...
function evalFun(fn, args, withNew, cn) {
  ...
  while(true) {
    try {
      ...
      if (pelm2 === 0) {
        pelm1 = {args : args, ts : timestamp, recurflag : false};
        addPending(fn, pelm1);
        ++noSum;
      }
      else if (typeof pelm2 === "number") {
      ...
      else /* pelm2 is an object {args, ts, recurflag} */ {
        ...
        if (pelm2.recurflag)
          ...
          pelm1 = {args : args, ts : timestamp, recurflag : true};
          addPending(fn, pelm1);
          ++noSum;
        }
      }
...
function cfa2(ast) {
  ...
  if (commonJSmode) { // create the exports object
    var e = new Aobj({}), eaddr = newCount();
    var eav = makeObjAval(eaddr), eavaddr = newCount();
    heap[eaddr] = e;
    heap[eavaddr] = eav;
...
(function() {
  ...
  initDeclsInHeap_override(DOT, function(n) {
    if (n.type === DOT && n.children[1].value === "addEventListener-") {
      n.addr = newCount();
      ++eventClassif.total;
...
function initDOMObjs() {
  ...
  var chr = new Chrome({}), chraddr = newCount(), chrav = makeObjAval(chraddr);
  heap[chraddr] = chr;
  chrome_obj_av_addr = newCount();
  heap[chrome_obj_av_addr] = chrav;
  var chr2 = new Chrome({}), chr2addr=newCount(), chr2av=makeObjAval(chr2addr);
  heap[chr2addr] = chr2;
  var con = new Content({}), conaddr = newCount(), conav = makeObjAval(conaddr);
  heap[conaddr] = con;

  chr.updateProp("content-", chr2av);
  chr.updateProp("contentDocument-", conav);
  chr.updateProp("contentWindow-", chr2av);
  chr.updateProp("defaultView-", conav);
  chr2.updateProp("document-", conav);
  con.updateProp("documentElement-", conav);
  chr.updateProp("opener-", chrav);
  con.updateProp("opener-", conav);
  chr.updateProp("selectedBrowser-", chrav);
  ...
  var chrarr = new Aobj({}), chrarraddr = newCount();
  ...
  var conarr = new Aobj({}), conarraddr = newCount();
  ...
  function toNodeList(args) {
    return new Ans(args[0] instanceof Chrome ? chrarrav : conarrav);
  }
  ...
  go.updateProp("content-", chr2av);
  go.updateProp("contentWindow-", chr2av);
  go.updateProp("document-", chrav);
  go.updateProp("gBrowser-", chrav);
  go.updateProp("opener-", chrav);
  ...
  function aEL(args, withNew, callNode) {
    ...
    var ratorNode = callNode.children[0], raddr = ratorNode.addr, ec, kind, r;

    if (!ratorNode) return new Ans(BOTTOM);
    ec = eventClassif[raddr];
    if (ec) {
      if (!ec.analyzed) {
        ec.analyzed = true;
      ...
      ec = eventClassif[raddr] = {lineno : ratorNode.lineno,
                                  analyzed : true,
                                  result : undefined};
    else
      throw errorWithCode(CFA_ERROR, "Can't create eventClassif entry.");

    kind = eventKinds[evt.slice(0,-1)];
    if (kind === XUL)
      r = [evt, "chrome", "chrome", "safe"];
    else if (kind === undefined && !args[4] && evt !== "unknown-") {
